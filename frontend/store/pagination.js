          ...
          switch (typeof obj) {
            case "object":
              if (!obj) {
                ...
                res = {};
                for (let key in obj) res[key] = walker(obj[key]);
              }
              break;
            case "number":
              res = obj.toString();
    ...
    actions: {
      ...
      init(context) {
        ...
        if (options.syncFiltersWithRouteParams) {
          filters = getInitialFiltersFromRouteParams(
            initialFilters,
            options.routeParamsPrefix
          );
          ...
          removeRouteGuard = router.afterEach(() => {
            ...
            setTimeout(() => {
              ...
              if (
                !_.isEqual(
                  context.getters.nonEmptyFilters,
      ...
      updateFilters({ commit, dispatch, state, getters }, filters) {
        ...
        if (!_.isEqual(newFilters, oldFilters)) {
          ...
          if (options.syncFiltersWithRouteParams) {
            syncFiltersWithRouteParams(
              getters.normalizedFilters,
              options.routeParamsPrefix
            );
          }
          return dispatch("load");
...
function getFiltersFromQueryParams(filters, prefix) {
  ...
  return removePrefix(
    _.pick(allQueryParams, Object.keys(prefixedFilters)),
    prefix
  );
}
...
function syncFiltersWithRouteParams(filters, prefix) {
  ...
  const newQueryParams = _.omitBy(
    { ...allQueryParams, ...prefixedFilters },
    _.isEmpty
  );
